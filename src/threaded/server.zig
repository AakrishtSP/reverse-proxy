...
pub fn run(allocator: std.mem.Allocator, router: *Router, port: u16) !void {
    ...
    try posix.bind(listen_fd, @ptrCast(&bind_addr), @sizeOf(posix.sockaddr.in));
    try posix.listen(listen_fd, 128);

    while (true) {
        var peer: posix.sockaddr.in = undefined;
        var peer_len: posix.socklen_t = @sizeOf(posix.sockaddr.in);
        const conn_fd = posix.accept(listen_fd, @ptrCast(&peer), &peer_len, posix.SOCK.CLOEXEC) catch |err| {
...
fn connectTo(addr: net.IpAddress) !posix.fd_t {
    switch (addr) {
        ...
        .ip6 => |ip6| {
            const fd = try posix.socket(posix.AF.INET6, posix.SOCK.STREAM, posix.IPPROTO.TCP);
            errdefer posix.close(fd);
            ...
            try posix.connect(fd, @ptrCast(&sa), @sizeOf(posix.sockaddr.in6));
            return fd;
        },
    }
}
...
fn writeAll(fd: posix.fd_t, bytes: []const u8) !void {
    ...
    while (sent < bytes.len) {
        sent += try posix.write(fd, bytes[sent..]);
    }
}
